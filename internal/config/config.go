...
type Config struct {
	...
	DBBackend string
	// DBFlavor distinguishes TiDB-specific SQL from plain MySQL when DBBackend="tidb".
	DBFlavor string
	...
	// Single-tenant bootstrap for self-hosted deployments.
	BootstrapTenantEnabled bool
	BootstrapTenantID      string
	BootstrapTenantName    string
	BootstrapTenantDBHost  string
	BootstrapTenantDBPort  int
	BootstrapTenantDBUser  string
	BootstrapTenantDBPass  string
	BootstrapTenantDBName  string
	BootstrapTenantDBTLS   bool
}
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		DBFlavor:              envOr("MNEMO_DB_FLAVOR", "tidb"),
		...
		DBConnectMaxRetries:   envInt("MNEMO_DB_CONNECT_MAX_RETRIES", 30),
		DBConnectRetryDelay:   envDuration("MNEMO_DB_CONNECT_RETRY_DELAY", 2*time.Second),
		BootstrapTenantEnabled: envBool("MNEMO_BOOTSTRAP_TENANT_ENABLED", false),
		BootstrapTenantID:      envOr("MNEMO_BOOTSTRAP_TENANT_ID", "default"),
		BootstrapTenantName:    envOr("MNEMO_BOOTSTRAP_TENANT_NAME", "Default Space"),
		BootstrapTenantDBHost:  envOr("MNEMO_BOOTSTRAP_TENANT_DB_HOST", "127.0.0.1"),
		BootstrapTenantDBPort:  envInt("MNEMO_BOOTSTRAP_TENANT_DB_PORT", 3306),
		BootstrapTenantDBUser:  envOr("MNEMO_BOOTSTRAP_TENANT_DB_USER", "root"),
		BootstrapTenantDBPass:  os.Getenv("MNEMO_BOOTSTRAP_TENANT_DB_PASS"),
		BootstrapTenantDBName:  envOr("MNEMO_BOOTSTRAP_TENANT_DB_NAME", "mnemos"),
		BootstrapTenantDBTLS:   envBool("MNEMO_BOOTSTRAP_TENANT_DB_TLS", false),
	}
	...
	switch cfg.DBFlavor {
	case "tidb", "mysql", "postgres", "db9":
		// ok
	default:
		return nil, fmt.Errorf("unsupported MNEMO_DB_FLAVOR %q; valid values are \"tidb\", \"mysql\", \"postgres\", and \"db9\"", cfg.DBFlavor)
	}
