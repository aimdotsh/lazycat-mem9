...
func (s *TenantService) InitSchemaForTenant(ctx context.Context, t *domain.Tenant) error {
	return s.initSchema(ctx, t)
}
...
func (s *TenantService) initSchema(ctx context.Context, t *domain.Tenant) error {
	...
	case "db9":
		...
		return nil
	case "tidb":
		mysqlCompat := s.dbFlavor == "mysql"
		t0 := time.Now()
		schema := buildMemorySchema(s.autoModel, s.autoDims)
		if mysqlCompat {
			schema = buildMySQLMemorySchema()
		}
...
func buildMySQLMemorySchema() string {
